...
using ENBDGroup.Foundation.Common.Redirect.Extensions;
...
namespace ENBDGroup.Foundation.Common.Redirect.Pipelines.HttpRequest
{
    public class RedirectMapResolver : HttpRequestProcessor
    {
        private string AllMappingsPrefix
        {
            get
            {
                return string.Format("{0}AllMappings-{1}-{2}-{3}", "LivApp-Foundation-Redirects-", Context.Database.Name, Context.Site.Name, Context.Language.Name);
            }
        }
                ...
                if (redirectMappings == null)
                {
                    ...
                    if (item1 != null)
                    {
                        Item[] array = (
                            ...
                            where i.InheritsFrom(ENBDGroup.Foundation.Common.Redirect.Templates.RedirectMap.ID)
        ...
        private string ResolvedMappingsPrefix
        {
            get
            {
                return string.Format("{0}ResolvedMappings-{1}-{2}-{3}", "LivApp-Foundation-Redirects-", Context.Database.Name, Context.Site.Name, Context.Language.Name);
            }
        }
        ...
        protected virtual string GetTargetUrl(RedirectMapping mapping, string input)
        {
            ...
            {
                target = string.Concat(target.TrimEnd(new char[] { '/' }), new HtmlSanitizer().Sanitize(HttpContext.Current.Request.Url.Query));
            }
            if (target.StartsWith("http://") || target.Contains("https://"))
            {
        ...
        protected virtual void Redirect301(HttpResponse response, string url)
        {
            ...
            url = new HtmlSanitizer().Sanitize(url);
